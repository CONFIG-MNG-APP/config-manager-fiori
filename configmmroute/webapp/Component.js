sap.ui.define(
    ...
    function (AppComponent, Element, Condition) {
        ...
        return AppComponent.extend("zcapstone.configmmroute.Component", {
            metadata: {
                manifest: "json"
            },
                    ...
                    var iInterval = setInterval(function () {
                        ...
                        if (!oFilterBar) {
                            if (iAttempt >= 200) {
                                ...
                                console.warn("[MMRoute] FilterBar not found after 20s");
                            }
                            return;
                        }
                        ...
                        var fnApply = function () {
                            try {
                                if (typeof oFilterBar.setFilterConditions === "function") {
                                    ...
                                    console.log("[MMRoute] Filters applied via setFilterConditions — ReqId:", sReqId, "ConfId:", sConfId);
                                    return;
                                }
                                ...
                                if (!oConditionModel) {
                                    if (iRetry++ < 20) {
                                        ...
                                        console.warn("[MMRoute] $filters model not available after retries");
                                    }
                                    return;
                                }
                                oConditionModel.setConditions(oConditions);
                                oFilterBar.triggerSearch();
                                console.log("[MMRoute] Filters applied via $filters model — ReqId:", sReqId, "ConfId:", sConfId);
                            } catch (e) {
                                console.error("[MMRoute] Error applying filter:", e);
                            }
                        };
