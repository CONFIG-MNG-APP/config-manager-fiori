sap.ui.define(
    ...
    function (AppComponent, Element, Condition) {
        ...
        return AppComponent.extend("zcapstone.configsdprice.Component", {
            metadata: {
                manifest: "json"
            },
                    ...
                    var iInterval = setInterval(function () {
                        ...
                        if (!oFilterBar) {
                            if (iAttempt >= 200) {
                                ...
                                console.warn("[SDPrice] FilterBar not found after 20s");
                            }
                            return;
                        }
                        ...
                        var fnApply = function () {
                            try {
                                // Ưu tiên public MDC API — không phụ thuộc vào $filters model
                                // mà FE gắn vào FilterBar SAU khi waitForInitialization() resolve
                                if (typeof oFilterBar.setFilterConditions === "function") {
                                    oFilterBar.setFilterConditions(oConditions);
                                    oFilterBar.triggerSearch();
                                    console.log("[SDPrice] Filters applied via setFilterConditions — ReqId:", sReqId, "ConfId:", sConfId);
                                    return;
                                }
                                // Fallback: $filters model (retry nếu chưa sẵn sàng)
                                var oConditionModel = oFilterBar.getModel("$filters");
                                if (!oConditionModel) {
                                    if (iRetry++ < 20) {
                                        setTimeout(fnApply, 300);
                                    } else {
                                        console.warn("[SDPrice] $filters model not available after retries");
                                    }
                                    return;
                                }
                                oConditionModel.setConditions(oConditions);
                                oFilterBar.triggerSearch();
                                console.log("[SDPrice] Filters applied via $filters model — ReqId:", sReqId, "ConfId:", sConfId);
                            } catch (e) {
                                console.error("[SDPrice] Error applying filter:", e);
                            }
                        };
